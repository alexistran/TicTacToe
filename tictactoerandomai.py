...
class TicTacToe():
	...
	def make_move(self):
		...
		elif self.curr_player == "Computer":
			print("Computer's turn")
			position = self.computer_player_move()
...
def main():
		...
		while gameNotEnd:
			if game.is_board_full() and game.is_there_a_win == False:
				print("The game has ended. There is no winner :(")
				break
			else:
				game.make_move()
				game.display_board()
				if game.is_there_a_win() and game.curr_player == "Player 1":
					print("Congratulations, " + game.curr_player + " has won!!!")
					break
				elif game.is_there_a_win() and game.curr_player == "Computer":
					print("Computer has beat you!")
					break
